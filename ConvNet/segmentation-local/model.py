    ...
    def predict(self):
        ...
        for step in range(self.conf.test_num_steps):
            ...
            with zipfile.ZipFile(datei) as z:
                ...
                sparray = spdf.values
                z.close()
            if not (sparray.shape[0]==groesse[0] and sparray.shape[1]==groesse[1]):
                print("ERROR WITH IMAGE: ",img_name,"\tThey dont have the same size")
                continue
            ...
            for x in range(groesse[0]):
                for y in range(groesse[1]):
                    ...
                    for klasse in range(groesse[2]):
                        preddict[superpixel][klasse] += predmatrix[x,y,klasse]
            ...
            with open(self.conf.out_dir + '/visual_prediction/probsp_eins/'+img_name+".csv", 'w') as f:
                f.write(towrite)
            if step < 100:
